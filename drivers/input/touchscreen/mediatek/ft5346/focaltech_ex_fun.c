...
unsigned char ft5x46_ctpm_LockDownInfo_get_from_boot(struct i2c_client *client, char *pProjectCode)
{
	...
	fts_read_reg(client, 0xA8, &uc_tp_vendor_id);
	printk("[tpd]ft5x46_ctpm_LockDownInfo_get_from_boot, uc_tp_vendor_id=%x\n", uc_tp_vendor_id);
	i_ret = hidi2c_to_stdi2c(client);
	if (i_ret == 0)
		printk("[tpd]HidI2c change to StdI2c fail ! \n");
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		if (i_ret < 0)
		{
			printk("[tpd]failed writing  0x55 and 0xaa ! \n");
			continue;
		}
		msleep(10);
	...
	{
		printk("[tpd]ft5x46_ctpm_LockDownInfo_get_from_boot, i=%d\n", i);
		return -EIO;
	}
  
	msleep(10);
	
	printk("[tpd]ft5x46_ctpm_LockDownInfo_get_from_boot, msleep(10)\n");
	...
	printk("[tpd]Step 5: reset the new FW\n");
